...
def check_output(flogname,N,message):
    ...
    for line in lineList:
        if message in line: issuccess = True

    return issuccess

def sel_intfiles(filelist,sdate,edate):

    d1 = dt.date(np.int(sdate[0:4]),np.int(sdate[4:6]),np.int(sdate[6:8]))
    d2 = dt.date(np.int(edate[0:4]),np.int(edate[4:6]),np.int(edate[6:8]))

    years=np.array([fname.split(":")[-1].split("-")[0] for fname in filelist],np.int)
    months=np.array([fname.split(":")[-1].split("-")[1] for fname in filelist],np.int)
    days=np.array([fname.split(":")[-1].split("-")[2][0:2] for fname in filelist],np.int)
...
def sel_grbfiles(filelist,sdate,edate):
    ...
    years=np.array([fname.split("_")[-1][0:4] for fname in filelist],np.int)
    months=np.array([fname.split("_")[-1][4:6] for fname in filelist],np.int)
    days=np.array([fname.split("_")[-1][6:8] for fname in filelist],np.int)
...
indeck_wrf = "namelist_input_EPICC_2km_ERA5_HVC_GWD.deck"

this_dir = os.getcwd()
wrf_dir = "/home/dargueso/WRF_runs/EPICC_WRFV4.2.2/WRF/run"
wps_dir = "/home/dargueso/WRF_runs/EPICC_WRFV4.2.2/WPS"
grb_dir = "/vg6/dargueso-NO-BKUP/BDY_DATA/ERA5/"
...
while (year < end_year or (year == end_year and month < end_month)):
    ...
    if (run_geogrid == True) and (firstday == True):
        ...
        if completed:
            ...
            raise Exception("ERROR: geogrid program did not finish successfully, check %s" %(flogname))
    ...
    if (run_ungrib == True):
        ...
        subprocess.call("./link_grib.csh %s" %(" ".join(str(x) for x in grbfiles)),shell=True)
        ...
        if completed:
            ...
            raise Exception("ERROR: ungrib program did not finish successfully, check %s" %(flogname))
    ...
    if (run_metgrid == True):
        ...
        if completed:
            ...
            raise Exception("ERROR: metgrid program did not finish successfully, check %s" %(flogname))
    ...
    if (run_real == True):
        ...
        if completed:
            ...
            raise Exception("ERROR: real program did not finish successfully, check %s" %(flogname))
        ...
        for bdy_file in bdy_filelist:
            ...
            subprocess.call("mv %s/%s %s/%s_%s-%s-%s" %(wrf_dir,bdy_file,bdy_dir,bdy_file,s_simyea,smonstr,sdaystr), shell=True)
