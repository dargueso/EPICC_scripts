...
def calculate_annual_cycle(mod_mem, varname):
    ...
    if not os.path.exists(
        f"AnnualCycle/{varname}_{model}_{member}_historical_1985-2014_AnnualCycle.nc"
    ):
        ctime00 = checkpoint(0)
        fin_p = finall.sel(time=slice("1985", "2014"))
    ...
    if not os.path.exists(
        f"AnnualCycle/{varname}_{model}_{member}_ssp585_2070-2099_AnnualCycle.nc"
    ):
        ctime00 = checkpoint(0)
        fin_f = finall.sel(time=slice("2070", "2099"))
...
def calculate_CC_signal(mod_mem, varname):
    ...
    if not os.path.exists(
        f"AnnualCycle_change/{varname}_{model}_{member}_CC_2070-2099_1985-2014_AnnualCycle.nc"
    ):
        ...
        datelist = pd.date_range(f"1985-01-01", periods=12, freq="MS")
