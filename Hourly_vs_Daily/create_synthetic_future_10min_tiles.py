...
def process_tile(wrun, ytile, xtile, min_ytile, max_ytile, min_xtile, max_xtile):
    ...
    output_file = f'{cfg.path_out}/{wrun}/future_synthetic_quant_per_sample_10min_{ytile}y-{xtile}x_{buffer_lab}.nc'
    ...
    try:
        ...
        ds_dist = (
            ...
            f"rainfall_probability_optimized_conditional_10min_{ytile}y-{xtile}x_{buffer_lab}.nc"
        )
        
        # Load the rainfall probability dataset
        rainfall_probability = xr.open_dataset(ds_dist, cache=False)

        ## Calculation of synthetic future interval data
        ...
        wet_interval_dist_present = rainfall_probability.wet_interval_distribution.values.copy()
        interval_intensity_dist_present = rainfall_probability.interval_distribution.values.copy()
        ...
        wet_interval_dist_present = np.nan_to_num(wet_interval_dist_present, nan=0.0)
        ...
        whdp_weighted = wet_interval_dist_present * samples_per_bin_present[:, np.newaxis, :, :]
        ...
        interval_cdf = np.cumsum(comp_wHr, axis=1).astype(np.float32, order="C")

        # Clean up intermediate arrays
        del wet_interval_dist_present, interval_intensity_dist_present, samples_per_bin_present
        ...
        for sample in range(cfg.n_samples):
            ...
            sample_quantiles = sf.generate_quantiles_directly(
                rain_arr, wet_cdf, interval_cdf, bin_idx, 
        ...
        future_synthetic_quant = xr.Dataset(
            ...
            attrs={
                'description': 'Synthetic future interval rainfall quantiles per sample',
                'units': 'mm/interval',
                'wet_threshold_daily': cfg.WET_VALUE_D,
                'wet_threshold_interval': cfg.WET_VALUE_H,
                'note': 'Quantiles calculated directly from synthetic interval values, only for values > wet_threshold. Edge tiles padded with NaN.',
                'tile_size': tile_size,
        ...
        confidence_file = f'{cfg.path_out}/{wrun}/future_synthetic_quant_confidence_10min_{ytile}y-{xtile}x_{buffer_lab}.nc'
