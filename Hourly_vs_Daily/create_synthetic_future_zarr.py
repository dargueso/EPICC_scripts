...
def process_tile(tile_info):
    ...
    # Add buffer (except at edges)
    y_start_buf = max(0, y_start - BUFFER)
    y_end_buf = min(rain_arr.shape[1], y_end + BUFFER)
    x_start_buf = max(0, x_start - BUFFER)
    x_end_buf = min(rain_arr.shape[2], x_end + BUFFER)
    ...
    # Inner region (no buffer) relative to buffered tile
    ...
    if iy_tile == ny_tiles - 1:
        iy1 = rain_tile.shape[1]
    if ix_tile == 0:
        ix0 = 0
    if ix_tile == nx_tiles - 1:
        ix1 = rain_tile.shape[2]
    ...
    for sample in range(N_SAMPLES):
        sample_quantiles = generate_quantiles_for_tile(
            ...
            seed=123 + sample + iy_tile * 1000 + ix_tile)
...
def main():
    ...
    # Get data arrays
    rain_low_freq = ds_future.RAIN.where(ds_future.RAIN > WET_VALUE_LOW).values.astype(np.float32)
    lat = ds_future.lat.isel(time=0).values
    lon = ds_future.lon.isel(time=0).values
    ...
    ds_output = xr.Dataset(
        ...
        attrs={
            ...
            'freq_low': FREQ_LOW,
            'n_samples': N_SAMPLES,
            'note': f'Quantiles calculated from synthetic {FREQ_HIGH} values > {WET_VALUE_HIGH} mm'
        }
    )
