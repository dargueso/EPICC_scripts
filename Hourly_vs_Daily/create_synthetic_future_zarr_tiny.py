...
def main():
    ...
    hourly_intensity_quantiles = np.zeros((len(BOOTSTRAP_QUANTILES),len(QUANTILES), ny, nx), dtype=np.float32)
    max_hourly_intensity_quantiles = np.zeros((len(BOOTSTRAP_QUANTILES),len(QUANTILES), ny, nx), dtype=np.float32)
