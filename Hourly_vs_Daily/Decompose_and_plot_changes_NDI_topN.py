...
topN = 20
output_filename = f"cumul_top{topN}.png"
...
Np = topN
Ip = finp.cumul.isel(event=slice(0,topN)).mean(dim='event')
Dp = finp.cumul_duration.isel(event=slice(0,topN)).mean(dim='event')
Nf = topN
If = finf.cumul.isel(event=slice(0,topN)).mean(dim='event')
Df = finf.cumul_duration.isel(event=slice(0,topN)).mean(dim='event')

Dhours = Df*Nf- Dp*Np


Dtotpr = finf.cumul.isel(event=slice(0,topN)).sum('event') - finp.cumul.isel(event=slice(0,topN)).sum('event')
DN = 0
DI = finf.cumul.isel(event=slice(0,topN)).mean(dim='event') - finp.cumul.isel(event=slice(0,topN)).mean(dim='event')
DD = finf.cumul_duration.isel(event=slice(0,topN)).mean(dim='event') - finp.cumul_duration.isel(event=slice(0,topN)).mean(dim='event')
...
mylevels_diff = np.arange(-400, 440, 40)
...
mylevels_hours = np.arange(-400, 440, 40)
...
cb1 = plt.colorbar(
    m0, ticks=np.arange(-400, 440, 40), cax=cbar_ax, orientation="horizontal", shrink=0.62
)
