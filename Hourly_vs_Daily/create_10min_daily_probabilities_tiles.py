...
def calculate_gini_coefficient(rainfall_data):
    ...
    # Check if we have complete days (144 10-min intervals per day)
    if total_10min % 144 != 0:
        # Truncate to complete days
        complete_10min = (total_10min // 144) * 144
        rr_arr = rr_arr[:complete_10min, :, :]
        total_10min = complete_10min
    
    ndays = total_10min // 144
    
    # Reshape to (ndays, 144, y, x) - group 10min intervals into days
    rr_reshaped = rr_arr.reshape((ndays, 144, y, x))
    ...
    # Create rank array for the 144 10-min intervals
    r = np.arange(1, 145)  # ranks 1 to 144
    
    # Reshape r to broadcast: (1, 144, 1, 1)
    r = r.reshape(1, 144, 1, 1)
...
def calculate_wet_interval_intensity_distribution_optimized(ds_m_wet_days, ds_d, wet_interval_fraction,
                                                      drain_bins=DRAIN_BINS, mrain_bins=MRAIN_BINS):
    print("Computing rainfall distributions...")
    
    ny = ds_m_wet_days.sizes['y']
    nx = ds_m_wet_days.sizes['x']
    ...
    wet_interval_distribution_bin = np.zeros((n_drain_bins, 144, ny, nx))
    ...
    for ibin in range(n_drain_bins):
        ...
        bin_days_intervals = bin_days.reindex(time=ds_m_wet_days.time, method='ffill')
        bin_ds_m_masked = ds_m_wet_days.where(bin_days_intervals)
        
        wet_intervals = bin_ds_m_masked.where(bin_ds_m_masked > 0).count(dim=['time'])
        total_intervals_in_bin = bin_days_intervals.sum(dim=['time'])
        ...
        bin_wet_intervals = wet_interval_fraction.where(bin_days)

        # Gini coefficient
        gini_per_day = calculate_gini_coefficient(bin_ds_m_masked) 
        ...
        # Histogram for wet intervals per day
        masked_wet_intervals = bin_wet_intervals * 144.0
        ...
        hist_wet_intervals = xr.apply_ufunc(
            ...
            input_core_dims=[["time"]],
            output_core_dims=[["bin"]],
            kwargs={"bins": np.arange(1, 146, 1)},
            vectorize=True,
            dask="parallelized" if use_dask_wet else "forbidden",
            output_dtypes=[float],
            dask_gufunc_kwargs={"output_sizes": {"bin": 144}} if use_dask_wet else {},
        )
...
file_pattern = f'{PATH_IN}/{wrun}/split_files_tiles_50/UIB_10MIN_RAIN_20??-??_{TILE_ID}.nc'
output_file = f'{PATH_OUT}/{wrun}/rainfall_probability_optimized_conditional_10min_{TILE_ID}.nc'

print(f"Processing tile {TILE_ID}")
print("Loading data...")

files = sorted(glob(file_pattern))
chunk_dict = {'time': 144*7, 'y': -1, 'x': -1}

ds = xr.open_mfdataset(files, combine='by_coords', parallel=True, chunks=chunk_dict)

ds_m = ds.RAIN.where(ds.RAIN > WET_VALUE_M, 0.0)
ds_d = ds_m.resample(time='1D').sum()
ds_d = ds_d.where(ds_d > WET_VALUE_D)

wet_days = ds_d > WET_VALUE_D
wet_days_intervals = wet_days.reindex(time=ds_m.time, method='ffill')
ds_m_wet_days = ds_m.where(wet_days_intervals)

wet_interval_fraction = ds_m_wet_days.where(ds_m_wet_days > 0).resample(time='1D').count() / 144.0
wet_interval_fraction = wet_interval_fraction.where(wet_interval_fraction > 0.0)

print("Loading into memory...")
ds_m_wet_days = ds_m_wet_days.load()
ds_d = ds_d.load()
...
interval_dist, wet_interval_dist, samples, gini_coeff = calculate_wet_interval_intensity_distribution_optimized(
    ds_m_wet_days, ds_d, wet_interval_fraction, DRAIN_BINS, MRAIN_BINS
)
...
interval_vec = np.arange(1, 145)  # 1 to 144
