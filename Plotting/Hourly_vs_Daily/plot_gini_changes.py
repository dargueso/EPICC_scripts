...
# Output figure path
FIG_OUT = '/home/dargueso/Analyses/EPICC/Hourly_vs_Daily/gini_changes.png'
...
def load_gini(wrun):
    path = f'{PATH_IN}/{wrun}/gini_{FREQ_HIGH}_given_{FREQ_LOW}.nc'
...
def weighted_mean_gini(ds, mask=None):
    ...
    if mask is not None:
        ...
        w    = w.where(mask)
    return gini.weighted(w).mean(dim=['y', 'x'])
