...
# Add tile grid (50x50 grid points)
tile_size = 50
# Use ceiling division to include partial tiles at edges
n_tiles_x = (nx + tile_size - 1) // tile_size
n_tiles_y = (ny + tile_size - 1) // tile_size

# Draw vertical lines for tiles
for i in range(n_tiles_x + 1):
    x_idx = i * tile_size
    if x_idx < nx:
        axs[0].plot(to_np(lons[:, x_idx]), to_np(lats[:, x_idx]), 
                   color='purple', linewidth=1.5, transform=ccrs.PlateCarree(), zorder=104)
    elif i == n_tiles_x:
        # Draw final boundary line at the edge
        axs[0].plot(to_np(lons[:, -1]), to_np(lats[:, -1]), 
                   color='purple', linewidth=1.5, transform=ccrs.PlateCarree(), zorder=104)
...
for j in range(n_tiles_y + 1):
    y_idx = j * tile_size
    if y_idx < ny:
        axs[0].plot(to_np(lons[y_idx, :]), to_np(lats[y_idx, :]), 
                   color='purple', linewidth=1.5, transform=ccrs.PlateCarree(), zorder=104)
    elif j == n_tiles_y:
        # Draw final boundary line at the edge
        axs[0].plot(to_np(lons[-1, :]), to_np(lats[-1, :]), 
...
# Add tile labels at center of each tile
for j in range(n_tiles_y):
    for i in range(n_tiles_x):
        # Calculate center of tile (accounting for partial tiles at edges)
        y_start = j * tile_size
        ...
        center_lon = lons[center_y, center_x].values
        center_lat = lats[center_y, center_x].values
        
        # Add label [y, x]
        axs[0].text(center_lon, center_lat, f'[{j},{i}]', 
...
plt.savefig(f'/home/dargueso/Analyses/EPICC/Hourly_vs_Daily/EPICC_domain_with_regs_tiles.png',dpi=150)
