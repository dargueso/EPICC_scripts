...
ds_all = xr.concat([ds_p, ds_f], dim="experiment")
...
for loc in range(len(locs_names)):
    ...
    print(loc_name)
    ds = ds_all.assign_coords(experiment=["Present", "Future"]).isel(y=slice(locs_y_idx[loc]-buffer, locs_y_idx[loc]+buffer+1), x=slice(locs_x_idx[loc]-buffer, locs_x_idx[loc]+buffer+1))
    ...
    comp_samp = _window_sum(samples_per_bin_present, buffer)
    comp_wWet = _window_sum(whdp_weighted, buffer)
    comp_wHr = _window_sum(hidp_weighted, buffer)
