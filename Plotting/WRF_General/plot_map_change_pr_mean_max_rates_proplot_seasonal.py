...
for ns,season in enumerate(['DJF','MAM','JJA','SON']):
    ...
    xseas_pre = fin_pre[varname].groupby('time.season').max(dim='time').sel(season=season)
    xseas_fut = fin_fut[varname].groupby('time.season').max(dim='time').sel(season=season)
...
plt.savefig(f'{cfg.path_out}/WRF_General/Change_{varname}_{freq}_{sdate.strftime("%Y-%m-%d_%H-%M")}-{edate.strftime("%Y-%m-%d_%H-%M")}_{reg}_seasons.png',dpi=150)
