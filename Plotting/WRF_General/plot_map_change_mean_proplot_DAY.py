...
lmean = MaxNLocator(nbins=15).tick_values(0,15)
...
xseas_pre = fin_pre.RAIN.mean(dim=['time']).squeeze()
xseas_fut = fin_fut.RAIN.mean(dim=['time']).squeeze()
...
plt.savefig(f'{cfg.path_out}/WRF_General/Change_RAIN_DAY_mean_{cfg.syear}-{cfg.eyear}_{reg}.png',dpi=150)
