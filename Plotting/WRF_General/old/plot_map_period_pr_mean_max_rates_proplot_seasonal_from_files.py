...
lmean = MaxNLocator(nbins=15).tick_values(0,5)
lmax = MaxNLocator(nbins=15).tick_values(0,50)
...
for ns,season in enumerate(['DJF','MAM','JJA','SON']):
    print(season)
    finm = xr.open_dataset(f'{cfg.path_in}/{wrun}/{cfg.patt_in}_{freq}_{varname}_{sdate.year}-{edate.year}-mean-{season}.nc')
    ...
    dplot0= finm[varname].squeeze()*6*24.
    ...
    finx = xr.open_dataset(f'{cfg.path_in}/{wrun}/{cfg.patt_in}_{freq}_{varname}_{sdate.year}-{edate.year}-max-{season}.nc')
...
fig.colorbar(m0,length=0.7, loc='b',label='mm day-1',col=1)
