...
def calc_midmonth(year):
  ...
  tdiference=(dt.datetime(year,1,0o1,0o1)-dt.datetime(year-1,12,0o1,0o1)).total_seconds()/2
  midm_date=[dt.datetime(year-1,12,0o1,0o1)+dt.timedelta(seconds=tdiference)]+midm_date

  tdiference=(dt.datetime(year+1,2,0o1,0o1)-dt.datetime(year+1,1,0o1,0o1)).total_seconds()/2
  midm_date=midm_date+[dt.datetime(year+1,1,0o1,0o1)+dt.timedelta(seconds=tdiference)]
...
CMIP6anom_dir="/vg6/dargueso-NO-BKUP/BDY_DATA/CMIP6_ANOMALY/"
ERA5_dir="/home/dargueso/BDY_DATA/ERA5/ERA5_netcdf"
figs_path="/vg6/dargueso-NO-BKUP/BDY_DATA/CMIP6_ANOMALY/Figs"
...
file_ref=nc.Dataset("%s/era5_daily_sfc_20160101.nc" %(ERA5_dir),'r')
...
for y in range(nyears):
  ...
  for month in range(month_i,month_f+1):
    ...
    for day in range(1,calendar.monthrange(year,month)[1]+1):
        ...
        ferapl=nc.Dataset("%s/era5_daily_pl_%s%02d%02d.nc" %(ERA5_dir,year,month,day),'r')
        ferasfc=nc.Dataset("%s/era5_daily_sfc_%s%02d%02d.nc" %(ERA5_dir,year,month,day),'r')
          ...
          if filewrite==True:
            ...
            for var in vars3d:
              ...
              if var=='hus':
                ...
                varT=ferapl.variables['%s' %(vars3d_codes['ta'])][nt,::-1,:,:]
                varP=np.repeat(np.repeat(plvs,varT.shape[1]),varT.shape[2])
                varP=varP.reshape(varT.shape[0],varT.shape[1],varT.shape[2]) # varP is in Pa

                es=100.*fs.compute_es(varT) # the result of compute is in hPa
                es_mod=cp.deepcopy(es)
                es_mod[30:,:,:]=0 # saturation pressure equal zero in the stratosphere (levels over 20 hPa)
            ...
            for var in vars2d:
              ...
              if create_figs==True:
                ...
                for ii in range(3):
                  ...
                  if ii==2:
                    aa=vout[var][:]
                  figname=figs_path+'%s_%s_%s-%s-%s-%s.png' %(var,file_name[ii],Y,M,D,H)
                  plt.contourf(aa)
                  plt.colorbar()
                  plt.title(var+' ['+units+']')
                  plt.savefig(figname)
