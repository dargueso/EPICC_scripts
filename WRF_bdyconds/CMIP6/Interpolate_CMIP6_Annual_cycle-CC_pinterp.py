...
def main():
    ...
    for vn,varname in enumerate(variables):
        ...
        if not os.path.exists(f'{CMIP6anom_dir}/interp_plevs/{varname}_CC_signal_ssp585_2076-2100_1990-2014_pinterp.nc'):
            fin= xr.open_dataset(f'{CMIP6anom_dir}/{varname}_CC_signal_ssp585_2076-2100_1990-2014.nc')
            fin.reindex(plev=fin.plev[::-1])
            fin_pinterp = fin.interp(plev=era5_plev,kwargs={"fill_value": "extrapolate"})
            fin_pinterp.to_netcdf(f'{CMIP6anom_dir}/interp_plevs/{varname}_CC_signal_ssp585_2076-2100_1990-2014_pinterp.nc',unlimited_dims='time')
